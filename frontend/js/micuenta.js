...
function mostrarFavoritos() {
  ...
  lista.innerHTML = favoritos.length
    ? favoritos.map((fav, idx) => {
        ...
        return `
          <li data-idx="${idx}">
            <img class="fav-img" src="${imgSrc || 'frontend/images/default.jpg'}" alt="${fav.titulo || fav.nombre}">
...
document.getElementById("form-cargar-prenda").addEventListener("submit", function(e) {
  e.preventDefault();
  ...
  this.reset();
  setTimeout(() => document.getElementById("mensaje-carga").textContent = "", 3000);
});
